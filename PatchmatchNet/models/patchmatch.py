...
class Evaluation(nn.Module):
    ...
    def forward(
        ...
        if is_inverse:
            ...
            # img_loc_to_visit = (335//2, 222//2)
            # def tensor2formatstr(tensor:torch.Tensor) -> str:
            #     return np.array2string(tensor.detach().cpu().numpy().copy(), formatter={"float_kind": lambda x: f"{x:.9f}"})
            # print(f'depth hypothesis at {img_loc_to_visit}:\n {tensor2formatstr(1.0 / depth_sample[(slice(None), slice(None), ) + tuple(reversed(img_loc_to_visit))])}')
            # print(f'scores at {img_loc_to_visit}:\n {tensor2formatstr(score[(slice(None), slice(None), ) + tuple(reversed(img_loc_to_visit))])}')
            depth_sample = inverse_max_depth + depth_index / (num_depth - 1) * (inverse_min_depth - inverse_max_depth)
            depth_sample = 1.0 / depth_sample
            # print(score.shape)
            # print(f'Estimated depth at {img_loc_to_visit}:\n {tensor2formatstr(depth_sample[(slice(None),) + tuple(reversed(img_loc_to_visit))])}')
        else:
