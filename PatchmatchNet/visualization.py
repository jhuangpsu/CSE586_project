...
from captum.attr import GuidedGradCam, GuidedBackprop, LayerActivation, \
        Saliency, IntegratedGradients, LayerConductance
...
def visualiza_attributions(path, attributions, attr_preprocess=lambda x:x.permute(1, 2, 0).detach().cpu().numpy(), cmap='viridis', alpha=0.7):
    if path is not None:
...
def save_depth(args):
    ...
    dataset = MVSDataset(
        ...
        image_extension=args.img_extension
    )

    image_loader = DataLoader(
        dataset=dataset, batch_size=args.batch_size, shuffle=False, num_workers=4, drop_last=False)
    print(model)
    layer1 = model.feature.output3
    layer2 = model.feature.output2
    layer3 = model.feature.output1
    # print(layer)
    layer_act1 = LayerActivation(model, layer1)
    layer_act2 = LayerActivation(model, layer2)
    layer_act3 = LayerActivation(model, layer3)
    dict_layer_act = {'layer1': layer_act1, 'layer2': layer_act2, 'layer3': layer_act3}
    target_tensor = (240, 330)
    # target_tensor = to_cuda(target_tensor)

    def model_wrapper(ref_img, src_imgs, intrinsics, extrinsics, depth_min, depth_max):
        return model(ref_img, src_imgs, intrinsics, extrinsics, depth_min, depth_max)[0].squeeze(0)
    init_grad = LayerConductance(model, layer1)

    for batch_idx, sample in enumerate(image_loader):
        sample_cuda = to_cuda(sample)
        inputs = (sample_cuda["images"],
                sample_cuda["intrinsics"],
                sample_cuda["extrinsics"],
                sample_cuda["depth_min"],
                sample_cuda["depth_max"])
        activations = []
        # for key, val in dict_layer_act.items():
        #     activation = val.attribute(inputs)
        #     activation = activation.mean(axis=1, keepdim=True)
        #     activations.append(activation)
        # grad = init_grad.attribute(sample_cuda["images"][0], target=target_tensor, additional_forward_args=(inputs[0][1:], inputs[1], inputs[2], inputs[3], inputs[4],), n_steps=5)
        grad = init_grad.attribute(sample_cuda["images"][0], target=target_tensor, additional_forward_args=(inputs[0][1:], inputs[1], inputs[2], inputs[3], inputs[4],))
        grad = grad.mean(axis=1, keepdim=True)
        activations.append(grad)
        visualiza_attributions(None, activations)
        break
...
def check_geometric_consistency(
    ...
    return mask, depth_reprojected, dist
...
def filter_depth(args, scan: str = ""):
    ...
    for ref_view, src_views in pair_data:
        ...
        ref_img, original_h, original_w = read_image(
            os.path.join(args.input_folder, scan, "images/{:0>8}{}".format(ref_view, args.img_extension)), args.image_max_dim)
        ...
        for src_view in src_views:
            ...
            src_image, original_h, original_w = read_image(
                os.path.join(args.input_folder, scan, "images/{:0>8}{}".format(src_view, args.img_extension)), args.image_max_dim)
            ...
            geo_mask, depth_reprojected, dist = check_geometric_consistency(
                ref_depth_est,
                ref_intrinsics,
                ref_extrinsics,
                src_depth_est,
        ...
        def np_normalize(arr: np.ndarray):
            return (arr - arr.min()) / (arr.max() - arr.min()) * 255
        cv2.imwrite(os.path.join(args.output_folder, scan, "mask/{:0>8}_dist.png".format(ref_view)), np_normalize(dist))
        # cv2.imwrite(os.path.join(args.output_folder, scan, "mask/{:0>8}_rela.png".format(ref_view)), np_normalize(relative_depth))
        # cv2.imwrite(os.path.join(args.output_folder, scan, "mask/{:0>8}_xref.png".format(ref_view)), np_normalize(x_ref))
        # cv2.imwrite(os.path.join(args.output_folder, scan, "mask/{:0>8}_xreproj.png".format(ref_view)), np_normalize(x_reproj))
        ...
        if args.display:
            ...
            print(f'max depth: {ref_depth_est.max()}, min depth: {ref_depth_est.min()}')
            cv2.imshow("ref_depth", cv2.applyColorMap(cv2.convertScaleAbs(ref_depth_est, alpha=0.2), cv2.COLORMAP_JET))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--img_extension", type=str, default=".jpg", help="extension for the input image files")
