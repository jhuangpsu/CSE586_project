...
def save_depth():
    ...
    test_loader = DataLoader(test_dataset, args.batch_size, shuffle=args.eval_shuffle, num_workers=1, drop_last=True)
...
def check_geometric_consistency(depth_ref, intrinsics_ref, extrinsics_ref, depth_src, intrinsics_src, extrinsics_src):
    ...
    mask = np.logical_and(dist < 0.8, relative_depth_diff < 0.1)
    depth_reprojected[~mask] = 0

    return mask, depth_reprojected, x2d_src, y2d_src, dist, relative_depth_diff, x2d_reprojected, x_ref
...
def filter_depth(dataset_root, scan, out_folder, plyfilename):
    ...
    for ref_view, src_views in pair_data:
        ...
        for src_view in src_views:
            ...
            geo_mask, depth_reprojected, x2d_src, y2d_src, dist, relative_depth_diff, x2d_reprojected, x_ref = check_geometric_consistency(ref_depth_est, ref_intrinsics, ref_extrinsics,
                                                                      src_depth_est,
        ...
        def np_normalize(arr: np.ndarray):
            return (arr - arr.min()) / (arr.max() - arr.min()) * 255.0
        Image.fromarray(np.uint8(np_normalize(dist))).save(os.path.join(out_folder, "mask/{:0>8}_dist.png".format(ref_view)))
        Image.fromarray(np.uint8(np_normalize(relative_depth_diff))).save(os.path.join(out_folder, "mask/{:0>8}_relative_depth_diff.png".format(ref_view)))
        Image.fromarray(np.uint8(np_normalize(x2d_reprojected))).save(os.path.join(out_folder, "mask/{:0>8}_x2d_reprojected.png".format(ref_view)))
        Image.fromarray(np.uint8(np_normalize(x_ref))).save(os.path.join(out_folder, "mask/{:0>8}_x_ref.png".format(ref_view)))
